...
exports.details = async (req, res) => {
  ...
  try {
    ...
    const { recordset: D1 } = await pool.request().query`
        SELECT GSVC_I_D1.Value FROM GSVC_I_D1
        WHERE GSVC_I_D1.CERTNO = ${ct}
      `;
    const { recordset: D2 } = await pool.request().query`
        SELECT GSVC_I_D2.Value FROM GSVC_I_D2
        WHERE GSVC_I_D2.CERTNO = ${ct}
    `;

    const { recordset: D3 } = await pool.request().query`
        SELECT GSVC_I_D3.Value FROM GSVC_I_D3
        WHERE GSVC_I_D3.CERTNO = ${ct}
    `;
